...
def test_cost_tracking_adds_only_db_callbacks_when_prisma_set(monkeypatch):
    import litellm
    ...
    observed = {
        ...
        "shadow_eval_loggers": sum(type(cb).__name__ == "ShadowEvalLogger" for cb in litellm.callbacks),
        "prisma_was_set": True,
    }
    assert normalize(observed) == {
        "added_to_callbacks": 1,
        "added_to_async_success": 1,
        "shadow_eval_loggers": 0,
        "prisma_was_set": True,
    }
